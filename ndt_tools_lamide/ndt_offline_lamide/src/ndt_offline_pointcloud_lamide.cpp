...
#include "ndt_offline_lamide/PointCloudReader.hpp"
...
void saveCloud(int counter, const pcl::PointCloud<pcl::PointXYZL> &cloud) {
...
int main(int argc, char **argv){
	...
	ndt_map_pubb = nh.advertise<ndt_map_lamide::NDTMapRGBMsg>("ndtndtndtmap", 10);
...
//       ROS_ERROR_STREAM("Failed to open : " << gt_file << " || " << est_file << " || " << odom_file);
    ...
    for(int i=0; i<scanfiles.size(); i++) {
		...
		perception_oru::ndt_offline::PointCloudBagReader<pcl::PointXYZL> vreader(velodyne_config_file,
							bagfilename,
...
// 				}
	...
	// 			sensorpose_est_file << frame_time << " " << transformToEvalString(Todo * Ts);
		...
		if (save_map) {
          ...
          perception_oru::toRGBMessage(ndtslammer.map, mapmsg, "velodyne");
					ndt_map_pubb.publish<ndt_map_lamide::NDTMapRGBMsg>(mapmsg);
				}
