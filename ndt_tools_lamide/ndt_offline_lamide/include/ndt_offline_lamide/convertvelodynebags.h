...
//    return true;
  ...
  int addToImages(const pcl::PointCloud<velodyne_pointcloud_oru::PointXYZIR> &pnts, int width, int height, int startpixelx, int startpixely, const tf::Transform &T) {

    double resolution_factor = width / view_width_;
    for(size_t i = 0;i<pnts.size();i++){
      int y = height-pnts[i].ring-1;
      tf::Point pt;
pt.setValue(pnts[i].x - T.getOrigin()[0], pnts[i].y - T.getOrigin()[1], pnts[i].z - T.getOrigin()[2]);
