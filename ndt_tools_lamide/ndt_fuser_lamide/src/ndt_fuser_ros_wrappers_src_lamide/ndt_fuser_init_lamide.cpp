...
void perception_oru::ndt_fuser::initSensorPose(perception_oru::NDTFuserHMT& fuser, const std::string& robot_frame, const std::string& sensor_frame){
	...
	try {
		listener.waitForTransform(robot_frame, sensor_frame, ros::Time(0), ros::Duration(10.0) );
		listener.lookupTransform(robot_frame, sensor_frame, ros::Time(0), transform);
...
void perception_oru::ndt_fuser::initRobotPose(perception_oru::NDTFuserHMT& fuser, pcl::PointCloud< pcl::PointXYZL >& cloud, std::string& world_frame, std::string& robot_frame, bool preLoad)
{
	tf::TransformListener listener;
	tf::StampedTransform transform;
	try {
...
void perception_oru::ndt_fuser::initRobotPose2D(perception_oru::NDTFuserHMT& fuser, pcl::PointCloud< pcl::PointXYZL >& cloud, std::string& world_frame, std::string& robot_frame, bool preLoad)
{
	tf::TransformListener listener;
	tf::StampedTransform transform;
	try {
