...
namespace perception_oru {
    void NDTFuserHMT::initialize(Eigen::Affine3d initPos, pcl::PointCloud<pcl::PointXYZL> &cloud, bool preLoad)
    {
	...
	if(beHMT) {
        ...
        map->initialize(Tnow.translation()(0), Tnow.translation()(1), Tnow.translation()(2),
                        map_size_x, map_size_y, map_size_z, 0.6, 0.4, 0.2, 0.7, 0.3, 0.8, 1.0, 0.5);
        }
	}
	//#endif
     ...
     *
     */
    Eigen::Affine3d NDTFuserHMT::update(Eigen::Affine3d Tmotion, pcl::PointCloud<pcl::PointXYZL> &cloud)
    {
  ...
  if(doMultires) {
        ...
        mapLow.initialize(cx, cy, cz, 3 * map_size_x, 3 * map_size_y, map_size_z, 0.6, 0.4,
                          0.2, 0.7, 0.3, 0.8, 1.0, 0.5);
...
//		delete cell;
	...
	if(be2D) {
		...
		if((diff.translation().norm() > max_translation_norm ||
			...
			///////////////////ORU_FIXME: check if this works for error beams //////////////////////////

			pcl::PointCloud<pcl::PointXYZL> max_beams;
