#include <ndt_fuser_lamide/ndt_fuser.h>

namespace perception_oru
{
void NDTFuser::initialize(Eigen::Affine3d initPos, pcl::PointCloud<pcl::PointXYZL>& cloud)
{
    ...
    Tnow = initPos;
    map->initialize(Tnow.translation()(0), Tnow.translation()(1), Tnow.translation()(2), map_size_x,
                    map_size_y, map_size_z, 0.6, 0.4, 0.2, 0.7, 0.3, 0.8, 1.0, 0.5);
    map->addPointCloud(Tnow.translation(), cloud, 0.1, 100.0, 0.1);
    map->computeNDTCells(CELL_UPDATE_MODE_SAMPLE_VARIANCE, 1e5, 255, Tnow.translation());
    isInit = true;
    Tlast_fuse = Tnow;
    if (visualize)
    {
...
Eigen::Affine3d NDTFuser::update(Eigen::Affine3d Tmotion, pcl::PointCloud<pcl::PointXYZL>& cloud)
{
    if (!isInit)
    {
    ...
    if (be2D)
    {
        if (matcher2D.match(*map, ndlocal, Tinit, true))
        {
            ...
            {
                fprintf(stderr,
                        "****  NDTFuser -- ALMOST DEFINATELY A REGISTRATION FAILURE *****\n");
                Tnow = Tnow * Tmotion;
            }
            else
            {
            ...
            /*
               ...
               "<<t4-t1<<std::endl; FILE *ftmp = fopen("tmp.txt","a"); fprintf(ftmp,"%lf, ",t4-t3);
               fclose(ftmp);*/
        }
        else
        {
