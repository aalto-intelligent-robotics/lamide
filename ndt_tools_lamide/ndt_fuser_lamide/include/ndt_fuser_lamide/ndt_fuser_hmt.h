...
#ifndef NO_NDT_VIZ
#include <ndt_visualisation_lamide/ndt_viz.h>
#endif
...
// 	    visualize = true; //Redundant
  ...
  void setMotionParams(const perception_oru::MotionModel2d::Params &p) {
	 ...
	 * @param[in] initPos : center position of the map.
	 */
	void initialize(Eigen::Affine3d initPos, pcl::PointCloud<pcl::PointXYZL> &cloud, bool preLoad=false);

	/**
	 * @brief Set the initial position and set the first scan to the map and use tf for fixing the first pose
	 */
	void initialize(pcl::PointCloud<pcl::PointXYZL> &cloud, std::string& world_frame, std::string& robot_frame, bool preLoad=false);

	/**
	 *
	 *
	 */
	Eigen::Affine3d update(Eigen::Affine3d Tmotion, pcl::PointCloud<pcl::PointXYZL> &cloud);
