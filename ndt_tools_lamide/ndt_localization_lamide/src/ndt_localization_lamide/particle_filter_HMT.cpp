#include "ndt_localization_lamide/particle_filter_HMT.hpp"
#include "ros/ros.h"
perception_oru::particle_filter_HMT::particle_filter_HMT(
    perception_oru::NDTMapHMT* ndtMap_,
...
void perception_oru::particle_filter_HMT::InitializeNormal(double x,
                                                           double y,
                                                           double th,
                                                           double var)
{
...
void perception_oru::particle_filter_HMT::UpdateAndPredictEff(Eigen::Affine3d tMotion,
                                                              perception_oru::NDTMap* ndtLocalMap_,
...
void perception_oru::particle_filter_HMT::Predict2D(
...
void perception_oru::particle_filter_HMT::SIRUpdate()
{
    ...
    while (i < particleCloud.size())
    {
        if (k >= particleCloud.size())
            k = particleCloud.size() - 1;
        tmp[i] = particleCloud[k];
        tmp[i].probability = 1.0 / (double)particleCloud.size();
        i++;
    }
...
void perception_oru::particle_filter_HMT::GetRandomPoint(perception_oru::NDTCell* cell,
                                                         double& x,
                                                         double& y,
                                                         double& th)
{
