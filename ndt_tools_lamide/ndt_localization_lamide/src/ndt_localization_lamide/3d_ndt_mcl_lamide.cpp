...
void perception_oru::ndt_mcl_3d::updateAndPredict(Eigen::Affine3d Tmotion, pcl::PointCloud<pcl::PointXYZL> &cloud){
  ...
  if(forceSIR){
    ...
    double varP=0;
    for(int i = 0; i<pf.size();i++){
      varP += (pf.pcloud[i].probability - 1.0/pf.size())*(pf.pcloud[i].probability - 1.0/pf.size());
    }
    varP /= pf.size();
    varP = sqrt(varP);
...
void perception_oru::ndt_mcl_3d::predict(Eigen::Affine3d Tmotion) {
  Eigen::Vector3d tr = Tmotion.translation();
  Eigen::Vector3d rot = Tmotion.rotation().eulerAngles(0,1,2);
...
void perception_oru::ndt_mcl_3d::updateAndPredictEff(Eigen::Affine3d Tmotion, pcl::PointCloud<pcl::PointXYZL> &cloud, double subsample_level){
