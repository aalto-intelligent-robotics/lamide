...
class ndt_mcl_3d{
  ...
  perception_oru::NDTMap map; 		///<This is my map
	particle_filter_3d pf; 						///<This is the particle filter
	double resolution;
	 ...
	 * Constructor
	 */
  ndt_mcl_3d(double map_resolution, perception_oru::NDTMap &nd_map, double zfilter):
          SIR_varP_threshold(0.006), SIR_max_iters_wo_resampling(25)
  {
	...
	void updateAndPredict(Eigen::Affine3d Tmotion, pcl::PointCloud<pcl::PointXYZL> &cloud);

	void updateAndPredictEff(Eigen::Affine3d Tmotion, pcl::PointCloud<pcl::PointXYZL> &cloud, double subsample_level);
