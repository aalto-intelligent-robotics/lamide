...
#include "ndt_feature_finder_lamide/utils.hpp"
...
// 				map.getCellAtPoint(forward_point, cell);
				...
				//ORU_TODO TAKE CARE OF THAT CASE
				if(cell_collision != NULL){
					...
					for(auto it = cells_initialized.begin() ; it != cells_initialized.end() ; ++it){
						...
						for(int i = 0; i < 2 ; ++i){
							...
							ROS_DEBUG_STREAM("collision data:  " << x_cell_size << " and i " << i << " orientation " << orientation << std::cos(orientation + (i * M_PI) ) );
							...
							ROS_DEBUG_STREAM(collision_point(0) << " + (" << x_cell_size << " * " << std::cos(orientation + (i * M_PI) ) << " ) ");
							ROS_DEBUG_STREAM(collision_point(1) << " + (" << x_cell_size << " * " << std::sin(orientation + (i * M_PI) ) << " ) ");

							ROS_DEBUG_STREAM("point forward " << forward_point.x <<" " << forward_point.y << " " << forward_point.z);
							ROS_DEBUG_STREAM("point collision " << collision_point(0) <<" " << collision_point(1) << " " << collision_point(2));
...
//								std::cout << "FINAL ANGLES " << angle_between << " " << angle_between2<<std::endl;
								...
								if(	(angle_between < ( angle_between2 + 0.1 ) ) && ( angle_between > (angle_between2 - 0.1 ) ) ){
									ROS_DEBUG_STREAM("Pushing back " << orientation + (i * M_PI)<< " with " << angle_between << " " << angle_between2);
									angle_orientations.push_back(orientation + (i * M_PI));
									angles.push_back(angle_final);
								}
								else{
									ROS_DEBUG_STREAM("Wrong angle " << angle_between << " " << angle_between2);
								}
							}
							else{
...
// //
// // 			perception_oru::NDTCell* cell_collision;
// // 			pcl::PointXYZL collision_point_cell;
...
// 			std::cout << "RADIUS " << radius << "*" << neig_size << std::endl;
			pcl::PointXYZL point = cell.getCenter();
			double radius = max;
