...
Convert::Convert(ros::NodeHandle node, ros::NodeHandle private_nh, std::string const& node_name)
    : data_(new velodyne_rawdata_oru::RawData()),
      ...
      diagnostics_(node, private_nh, node_name)
{
    ...
    if (config_.organize_cloud)
    {
        container_ptr_ = boost::shared_ptr<OrganizedCloudXYZIR>(new OrganizedCloudXYZIR(
            config_.max_range, config_.min_range, config_.target_frame, config_.fixed_frame,
            config_.num_lasers, data_->scansPerPacket()));
    }
    else
    {
        container_ptr_ = boost::shared_ptr<PointcloudXYZIR>(
            new PointcloudXYZIR(config_.max_range, config_.min_range, config_.target_frame,
    ...
    srv_ =
        boost::make_shared<dynamic_reconfigure::Server<velodyne_pointcloud_oru::CloudNodeConfig>>(
            private_nh);
    dynamic_reconfigure::Server<velodyne_pointcloud_oru::CloudNodeConfig>::CallbackType f;
...
void Convert::callback(velodyne_pointcloud_oru::CloudNodeConfig& config, uint32_t level)
{
    ...
    container_ptr_->configure(config_.max_range, config_.min_range, config_.fixed_frame,
...
void Convert::processScan(const velodyne_msgs::VelodyneScan::ConstPtr& scanMsg)
{
    ...
    diagnostics_.update();
    auto cld = container_ptr_->finishCloud();
    //cld.header.stamp = ros::Time::now();
    output_.publish(cld);
    //ROS_INFO_STREAM("I'm custom");
}
