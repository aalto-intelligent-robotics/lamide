...
NDTD2DRegType::NDTD2DRegType(RegParamPtr paramptr) : registrationType(paramptr)
{
    ...
    if (param_ptr != NULL)
    {
        ...
        else
        {
            if (do_soft_constraints_)
            {
                cout << "Create matcher Soft Constraints" << endl;
                matcher3D_ = new NDTMatcherD2DSC(false);
            }
            else
            {
                cout << "Create matcher ndtd2d" << endl;
...
bool NDTD2DRegType::Register(MapTypePtr maptype,
                             Eigen::Affine3d& Tnow,
                             pcl::PointCloud<pcl::PointXYZL>& cloud,
                             Eigen::MatrixXd& Tcov)
{
    std::cout << "NDTD2D reg" << std::endl;
    NDTMap* globalMap;
...
bool NDTD2DRegType::RegisterMap2Map(MapTypePtr map_prev,
                                    MapTypePtr map_next,
...
bool NDTD2DRegType::Register(pcl::PointCloud<pcl::PointXYZL>& target,
                             Eigen::Affine3d& Tnow,
                             ...
                             Eigen::MatrixXd& Tcov)
{
    ...
    else if (!multires_)
    {
        ...
        if (do_soft_constraints_ && !registration2d_)
        {
            cout << "SC" << endl;
            registration_status =
                dynamic_cast<NDTMatcherD2DSC*>(matcher3D_)->match(*ndttar, *ndtsrc, Tnow, Tcov);
            // Cov = dynamic_cast<NDTMatcherD2DSC*>(matcher3D_)->Cov;
        }
        else if (do_soft_constraints_ && registration2d_)
        {
