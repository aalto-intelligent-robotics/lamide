...
void ScanPlot::plot(const sensor_msgs::LaserScan& scan,
                    ...
                    double alpha)
{
    ...
    std::cout << "derivative size:=" << der.size() << ", data size=" << data_set.size() << endl;
...
void ScanPlot::scanToPairVector(const sensor_msgs::LaserScan& scan,
                                ...
                                pair_vector_double& data)
{
    ...
    switch (rep)
    {
        ...
        case RangeAngleDerivative:
            ...
            {
                angle = -(scan.angle_min + (double)i / (double)(scan.ranges.size()) *
        ...
        default:
            for (int i = 0; i < scan.ranges.size(); i++)
            {
                ...
                range = scan.ranges[i];
                data.push_back(
                    std::make_pair(range * cos(angle + M_PI / 2), range * sin(angle + M_PI / 2)));
            }
    }
}
