...
/*void TestCellVector(){
  ...
  if(vek.GetVal(1,2,3)!=NULL)
    cout<<"and in the vector we find that the same cel is also modified
to=\n"<<*vek.GetVal(1,2,3)<<endl; else{ cerr<<"couldnt find the cell in the vector"<<endl; exit(0);
  }
...
public:
    testGraphMapSerialization()
    {
        ...
        GraphMapParamPtr graphpar = GraphMapParamPtr(new GraphMapParam());
        graphpar->GetParametersFromRos();
        graphMap = GraphMapPtr(new GraphMap(pose, mappar, graphpar));
        Eigen::Affine3d diff = Eigen::Affine3d::Identity();
        diff.translation() << 1, 2, 3;
        graphMap->AddMapNode(diff, unit_covar);
        saveGraphMap();
...
/*
  ...
  updated) Eigen::Matrix3d evecs_; // Eigen vectors Eigen::Vector3d mean_;  // Mean of the normal
  distribution Eigen::Vector3d evals_; // Eigen values CellClass cl_; static bool parametersSet_;
  // ??? static double EVAL_ROUGH_THR;		// = 0.1;								// ??? static
  double EVEC_INCLINED_THR; 	// = cos(8*M_PI/18);//10 degree slope;	// ??? static double
  EVAL_FACTOR;													// ??? double d1_,d2_; unsigned int
  N; 	///Number of points used for Normal distribution estimation so far
