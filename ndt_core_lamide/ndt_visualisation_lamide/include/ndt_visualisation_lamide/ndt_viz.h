...
#include <ndt_visualisation_lamide/NDTVizGlut.hh>
#include <pthread.h>
...
class NDTViz {
  ...
  void addGrid(size_t nbCells, double cellSize, double lineWidth = 1., double R = 1.0, double G = 0.4, double B = 0., double A = 0.8) {
	...
	/**
	  ...
	  */
	void addScan(Eigen::Vector3d orig, pcl::PointCloud<pcl::PointXYZL> &cloud, double R=1.0,double G=1.0,double B=1.0){
	...
	void addPointCloud(pcl::PointCloud<pcl::PointXYZL> &cloud, double R=1.0,double G=1.0,double B=1.0){
...
//	    for(unsigned int i=0;i<global_ndts.size();i++) delete global_ndts[i];
	...
	void plotLocalNDTMap(pcl::PointCloud<pcl::PointXYZL> &cloud, double resolution, double R=0, double G=1, double B=0, bool heightCoding=true){
	    if(win3D == NULL) return;
...
//	    }
	...
	void plotLocalConflictNDTMap(perception_oru::NDTMap *map, pcl::PointCloud<pcl::PointXYZL> &cloud,
