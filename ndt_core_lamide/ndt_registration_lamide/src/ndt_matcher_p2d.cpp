...
#include <ndt_registration_lamide/ndt_matcher_p2d.h>
...
void NDTMatcherP2D::generateScoreDebug(const char* out, pcl::PointCloud<pcl::PointXYZL>& fixed,
        pcl::PointCloud<pcl::PointXYZL>& moving)
{
...
bool NDTMatcherP2D::match( pcl::PointCloud<pcl::PointXYZL>& target,
...
bool NDTMatcherP2D::covariance( pcl::PointCloud<pcl::PointXYZL>& target,
        pcl::PointCloud<pcl::PointXYZL>& source,
        Eigen::Transform<double,3,Eigen::Affine,Eigen::ColMajor>& T,
                                                       ...
                                                       )
{
    ...
    pcl::PointCloud<pcl::PointXYZL> cloud = source;
    perception_oru::transformPointCloudInPlace(T,cloud);
...
//    double NORM_MAX = support_size/8, ROT_MAX = M_PI/18; //
    ...
    pcl::PointCloud<pcl::PointXYZL> prevCloud, nextCloud;
    prevCloud = source;
    nextCloud = source;
    T.setIdentity();
    TR.setIdentity();
...
//	cout<<"incr= ["<<pose_increment_v.transpose()<<"]"<<endl;
        ...
        prevCloud = perception_oru::transformPointCloud<pcl::PointXYZL>(T,source);
        scoreP = score;
        score = scorePointCloud(prevCloud,targetNDT);
	if(score < score_best)
	{
...
void NDTMatcherP2D::check( pcl::PointCloud<pcl::PointXYZL>& fixed,
            pcl::PointCloud<pcl::PointXYZL>& moving,
            Eigen::Transform<double,3,Eigen::Affine,Eigen::ColMajor>& T )
{
  // init
...
void NDTMatcherP2D::computeDerivatives(pcl::PointXYZL &pt)
{
...
double NDTMatcherP2D::scorePointCloud(pcl::PointCloud<pcl::PointXYZL> &source,
        NDTMap &targetNDT)
{
    double score_here = 0;
...
void NDTMatcherP2D::derivativesPointCloud(pcl::PointCloud<pcl::PointXYZL> &source,
        NDTMap &targetNDT,
...
double NDTMatcherP2D::lineSearchMT(  Eigen::Matrix<double,6,1> &score_gradient_init,
        ...
        pcl::PointCloud<pcl::PointXYZL> &sourceCloud,
        ...
        NDTMap &targetNDT)
{
    ...
    pcl::PointCloud<pcl::PointXYZL> cloudHere = sourceCloud;
...
pcl::PointCloud<pcl::PointXYZL> NDTMatcherP2D::subsample(pcl::PointCloud<pcl::PointXYZL>& original)
{
    ...
    if(subsampleType == "GRID")
    {
        ...
        pcl::PointCloud<pcl::PointXYZL> res;
