...
Eigen::MatrixXd computeHessianMahalanobis(const Eigen::Matrix3d &Q) {
  ...
  H << Q(0,0)+Q(0,0), Q(1,0)+Q(0,1), Q(2,0)+Q(0,2),
       Q(0,1)+Q(1,0), Q(1,1)+Q(1,1), Q(2,1)+Q(1,2),
      Q(0,2)+Q(2,0), Q(1,2)+Q(2,1), Q(2,2)+Q(2,2);
  return H;
}
double computeScoreMahalanobis(const Eigen::Matrix<double,3,1> &x,
                               const Eigen::Matrix3d &Q) {
  return x.transpose()*Q*x;
  //  return 0.;
}
Eigen::Matrix<double,3,1> computeGradientMahalanobis(const Eigen::Matrix<double,3,1> &x,
                                                     const Eigen::Matrix3d &Q) {
  return computeHessianMahalanobis(Q)*x;
}
double NDTMatcherD2DSC_2D::lineSearchMTSC(Eigen::Matrix<double,3,1> &increment,
                                          ...
                                          const Eigen::Transform<double,3,Eigen::Affine,Eigen::ColMajor> &T)
{
  ...
  double eulerAngles_z;
  Eigen::Matrix<double,3,1> X = localpose;
  ...
  while (1)
  {
    ...
    ps = Eigen::Translation<double,3>(pincr(0),pincr(1),0)*
        ...
        Eigen::AngleAxisd(pincr(2),Eigen::Vector3d::UnitZ()); //A step z,ex, ey not used when registration is locked in 2d.
    ...
    X<<localpose(0) + Tposition_diff(0),localpose(1) + Tposition_diff(1),localpose(2) + pincr(2);
...
/*void NDTMatcherD2DSC_2D::scoreComparision( const NDTMap& targetNDT,
                                           const NDTMap& sourceNDT,
                                           ...
                                           double alpha)
{
  Eigen::MatrixXd Hessian(3,3), score_gradient(3,1);
  const std::vector<NDTCell*> nextNDT = sourceNDT.pseudoTransformNDT(T*offset);
  score_NDT = derivativesNDT_2d(nextNDT,targetNDT,score_gradient,Hessian,false);
  Eigen::VectorXd X = ndt_generic::affine3dToVector(odom_offset);
  ndt_generic::normalizeEulerAngles6dVec(X);
  Eigen::Matrix3d Q = Tcov.inverse();
...
bool NDTMatcherD2DSC_2D::match( const NDTMap& targetNDT,
                                const NDTMap& sourceNDT,
                                Eigen::Transform<double,3,Eigen::Affine,Eigen::ColMajor>& T ,
                                const Eigen::Matrix3d& Tcov)
{
std::cout<<"2d registration sc"<<std::endl;
  //locals
  ...
  //double alpha = 0.95;
  double step_size = 1;
  Eigen::Matrix<double,3,1>  pose_increment_v, scg, pose_local_v, pose_increment_v_Tframe;
  Eigen::MatrixXd Hessian(3,3), score_gradient(3,1); //column vectors, pose_increment_v(6,1)
  ...
  Eigen::Array<double,3,1> weights;
  ...
  while(!convergence)
  {
    std::cout<<"registration Loop"<<std::endl;
    iter++;
    TR.setIdentity();
    Hessian.setZero();
    score_gradient.setZero();

    double score_here = derivativesNDT_2d(nextNDT,targetNDT,score_gradient,Hessian,true);
    X = pose_local_v;
    ...
    Eigen::SelfAdjointEigenSolver<Eigen::Matrix<double,3,3> > Sol (Hessian);
  ...
  score_gradient.setZero();
  double score_here = derivativesNDT_2d(nextNDT,targetNDT,score_gradient,Hessian,false);
  X = pose_local_v;
  score_here += alpha*computeScoreMahalanobis(X,Q);
  if(score_here > score_best)
  {
