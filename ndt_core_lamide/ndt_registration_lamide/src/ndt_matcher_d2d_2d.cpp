                            ...
                            std::vector<double> _resolutions)
{
    ...
    current_resolution = 0.1; // Argggg!!! This is very important to have initiated! (one day
                              // debugging later) :-) TODO do we need to set it to anything better?
    lfd1 = 1.0; // lfd1/(double)sourceNDT.getMyIndex()->size(); //current_resolution*2.5; //Changed
                // november 2017 from 1.0
    lfd2 = 0.05; // 0.1/current_resolution; ////Changed november 2017 from 0.05
    ITR_MAX = 50;
...
bool NDTMatcherD2D_2D::match(pcl::PointCloud<pcl::PointXYZL>& target,
                             ...
                             bool useInitialGuess)
{
    ...
    while (!convergence)
    {
        ...
        {
            std::cout << "\%gradient vanished at itr: " << itr_ctr
                      << ", norm : " << score_gradient_2d.norm() << std::endl;
            ...
            if (score_here > score_best)
            {
                std::cout << "crap iterations, best was " << score_best << " last was "
                          << score_here << std::endl;
                T = Tbest;
                finalscore = score_best;
                status_ = WRONG_DIRECTION;
                return false;
            }
            else
            {
        ...
        // H<<"];\n"<<std::endl;				  // tillfälligt bortkommenterad std::cout<<"gradh
        // (:,"<<itr_ctr+1<<")= ["<<scg.transpose()<<"];"<<std::endl;         //
...
bool NDTMatcherD2D_2D::update_gradient_hessian_local_2d(Eigen::MatrixXd& score_gradient,
...
void NDTMatcherD2D_2D::computeDerivativesLocal_2d(Eigen::Vector3d& x,
                                                  Eigen::Matrix3d C1,
                                                  ...
                                                  bool computeHessian)
{
    ...
    //_Zest
    _Zest.block<3, 3>(0, 6) << -2 * C1(0, 1), -C1(1, 1) + C1(0, 0), -C1(1, 2), -C1(1, 1) + C1(0, 0),
    ...
    if (computeHessian)
    {
        ...
        _ZHest.block<3, 3>(6, 6) << 2 * C1(1, 1) - 2 * C1(0, 0), -4 * C1(0, 1), -C1(0, 2),
            -4 * C1(0, 1), 2 * C1(0, 0) - 2 * C1(1, 1), -C1(1, 2), -C1(0, 2), -C1(1, 2), 0;
    }
}

void NDTMatcherD2D_2D::computeDerivatives_2d(Eigen::Vector3d& x,
                                             Eigen::Matrix3d C1,
                                             bool computeHessian)
{
    ...
    Eigen::Matrix3d myBlock;
    //_Zest
    Zest.block<3, 3>(0, 6) << -2 * C1(0, 1), -C1(1, 1) + C1(0, 0), -C1(1, 2), -C1(1, 1) + C1(0, 0),
        2 * C1(0, 1), C1(0, 2), -C1(1, 2), C1(0, 2), 0;
        ...
        for (unsigned int i = 0; i < sourceNDT.size(); i++)
        {
            ...
            for (unsigned int j = 0; j < cells.size(); j++)
            {
                ...
                if (cell->hasGaussian_)
                {
                    ...
                    {
                        std::cout << "! update gradient hessian" << std::endl;
                        continue;
                    }
                    score_here_loc += sh;
                    cell = NULL;
            ...
            // std::cout<<"gradient of thread: "<<thread_id<<" = "<<score_here_loc<<std::endl;
    ...
    for (unsigned int i = 0; i < sourceNDT.size(); i++)
    {
        ...
        for (int j = 0; j < cells.size(); j++)
        {
            ...
            if (cell->hasGaussian_)
            {
                ...
                // = ["<<CFixed<<"];\n"; 		    if(!update_gradient_hessian(score_gradient, Hessian,
...
int NDTMatcherD2D_2D::MoreThuente::cstep(double& stx,
                                         double& fx,
                                         double& dx,
                                         double& sty,
                                         double& fy,
