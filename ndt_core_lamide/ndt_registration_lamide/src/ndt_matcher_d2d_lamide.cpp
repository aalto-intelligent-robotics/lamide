...
void NDTMatcherLamide::init(bool _isIrregularGrid,
                         ...
                         std::vector<double> _resolutions)
{
    ...
    // ORU_TODO do we need to set it to anything better?
    current_resolution = 0.1;
    lfd1 = 1.0;  // lfd1/(double)sourceNDT.getMyIndex()->size(); //current_resolution*2.5;
...
bool NDTMatcherLamide::match(pcl::PointCloud<pcl::PointXYZL>& target,
                          ...
                          bool useInitialGuess)
{
    ...
    // initial guess
    pcl::PointCloud<pcl::PointXYZL> sourceCloud = source;
    ...
    time_combined =
        (tv_end0.tv_sec - tv_start0.tv_sec) * 1000. + (tv_end0.tv_usec - tv_start0.tv_usec) / 1000.;

    return ret;
}

bool NDTMatcherLamide::match(NDTMap& targetNDT,
                          NDTMap& sourceNDT,
                          Eigen::Transform<double, 3, Eigen::Affine, Eigen::ColMajor>& T,
                          bool useInitialGuess)
{
    std::cout << "step: " << step_control << std::endl;
    ...
    while (!convergence)
    {
        ...
        if (!convergence && itr_ctr > ITR_MAX)
        {
...
bool NDTMatcherLamide::update_gradient_hessian_local(Eigen::MatrixXd& score_gradient,
                                                  ...
                                                  bool computeHessian)
{
    ...
    {
        Hessian +=
            factor * (2 * _Jest.transpose() * B * _Jest + 2 * _xtBH - _xtBZhBx -
...
bool NDTMatcherLamide::update_gradient_hessian(Eigen::MatrixXd& score_gradient,
                                            Eigen::MatrixXd& Hessian,
                                            const Eigen::Vector3d& x,
                                            const Eigen::Matrix3d& B,
                                            const double& likelihood,
                                            bool computeHessian)
{
    ...
    if (computeHessian)
    {
        Hloc = factor * (2 * Jest.transpose() * B * Jest + 2 * xtBH - xtBZhBx -
...
void NDTMatcherLamide::computeDerivativesLocal(Eigen::Vector3d& x,
                                            Eigen::Matrix3d C1,
                                            ...
                                            bool computeHessian)
{
    ...
    //_Zest
    myBlock << 0, -C1(0, 2), C1(0, 1), -C1(0, 2), -2 * C1(1, 2), -C1(2, 2) + C1(1, 1), C1(0, 1),
        -C1(2, 2) + C1(1, 1), 2 * C1(1, 2);
    ...
    myBlock << -2 * C1(0, 1), -C1(1, 1) + C1(0, 0), -C1(1, 2), -C1(1, 1) + C1(0, 0), 2 * C1(0, 1),
        C1(0, 2), -C1(1, 2), C1(0, 2), 0;
    ...
    if (computeHessian)
    {
        ...
        //_ZHest
        myBlock << 0, -C1(0, 1), -C1(0, 2), -C1(0, 1), 2 * C1(2, 2) - 2 * C1(1, 1), -4 * C1(1, 2),
        ...
        myBlock << 0, C1(1, 2), C1(0, 0) - C1(1, 1), C1(1, 2), -2 * C1(0, 2), 2 * C1(0, 1),
            C1(0, 0) - C1(1, 1), 2 * C1(0, 1), 2 * C1(0, 2);
        _ZHest.block<3, 3>(9, 15) = myBlock;

        myBlock << 2 * C1(2, 2) - 2 * C1(0, 0), -C1(0, 1), -4 * C1(0, 2), -C1(0, 1), 0, -C1(1, 2),
            -4 * C1(0, 2), -C1(1, 2), 2 * C1(0, 0) - 2 * C1(2, 2);
        _ZHest.block<3, 3>(12, 12) = myBlock;

        myBlock << -2 * C1(1, 2), C1(0, 2), 2 * C1(0, 1), C1(0, 2), 0, C1(1, 1) - C1(0, 0),
...
void NDTMatcherLamide::computeDerivatives(Eigen::Vector3d& x, Eigen::Matrix3d C1, bool computeHessian)
{
...
double NDTMatcherLamide::scoreNDT(std::vector<NDTCell*>& sourceNDT, NDTMap& targetNDT)
{
...
double NDTMatcherLamide::scoreNDT_OM(NDTMap& sourceNDT, NDTMap& targetNDT)
{
    NUMBER_OF_ACTIVE_CELLS = 0;
    ...
    while (it != lz->end())
    {
        ...
        for (unsigned int j = 0; j < all_cells.size(); j++)
        {
            ...
            pcl::PointXYZL cen1 = cell->getCenter();
            pcl::PointXYZL cen2 = source->getCenter();
            ...
            // "<<ll*((1-o1)*(1-o2) - o1*(1-o2) -o2*(1-o1))<<std::endl; source->consistency_score +=
...

double NDTMatcherLamide::scoreNDTPositive(
...
double NDTMatcherLamide::derivativesNDT(const std::vector<NDTCell*>& sourceNDT,
...
>>>>>>> daniel-develop
                ...
                // = ["<<CFixed<<"];\n"; 		    if(!update_gradient_hessian(score_gradient,
                // Hessian,
                // transformed, Cinv, sh, computeHessian))
...
double NDTMatcherLamide::lineSearchMT(Eigen::Matrix<double, 6, 1>& increment,
                                   std::vector<NDTCell*>& sourceNDT,
                                   NDTMap& targetNDT)
{
...
int NDTMatcherLamide::MoreThuente::cstep(double& stx,
                                      double& fx,
                                      double& dx,
                                      double& sty,
                                      double& fy,
...
double NDTMatcherLamide::MoreThuente::min(double a, double b)
{
    return (a < b ? a : b);
}

double NDTMatcherLamide::MoreThuente::max(double a, double b)
{
    return (a > b ? a : b);
}

double NDTMatcherLamide::MoreThuente::absmax(double a, double b, double c)
{
    a = fabs(a);
    b = fabs(b);
    c = fabs(c);
...
double NDTMatcherLamide::normalizeAngle(double a)
{
...
/*
void NDTMatcherLamide<PointSource,PointTarget>::generateScoreDebug(const char* out,
pcl::PointCloud<pcl::PointXYZL>& fixed, pcl::PointCloud<pcl::PointXYZL>& moving) {
  ...
  for(double x=lmin; x<lmax; x+=lstep) {
      ...
      pcl::PointCloud<pcl::PointXYZL> cloud = moving;
...
bool NDTMatcherLamide::covariance(NDTMap& targetNDT,
                               NDTMap& sourceNDT,
                               Eigen::Transform<double, 3, Eigen::Affine, Eigen::ColMajor>& T,
                               Eigen::MatrixXd& cov)
{
    ...
    for (int i = 0; i < sourceNDTN.size(); i++)
    {
        ...
        if (cell->hasGaussian_)
        {
            ...
            {
                G(q) =
                    -transformed.transpose() * Q * Zest.block<3, 3>(0, 3 * q) * Cinv * transformed;
                G(q) = G(q) - transformed.transpose() * Cinv * Zest.block<3, 3>(0, 3 * q) * Q *
                                  transformed;
            }
    ...
    // std::endl; cov = cov.inverse(); removed by daniel
...
bool NDTMatcherLamide::covariance(pcl::PointCloud<pcl::PointXYZL>& target,
                               pcl::PointCloud<pcl::PointXYZL>& source,
                               Eigen::Transform<double, 3, Eigen::Affine, Eigen::ColMajor>& T,
                               Eigen::MatrixXd& cov)
{
    ...
    pcl::PointCloud<pcl::PointXYZL> sourceCloud = perception_oru::transformPointCloud(T, source);
