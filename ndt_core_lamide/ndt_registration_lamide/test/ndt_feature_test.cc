...
#include <ndt_registration_lamide/ndt_matcher_d2d_feature.h>
...
int main(int argc, char** argv)
{
    ...
    pcl::PointCloud<pcl::PointXYZL> static_pc, moving_pc, tmp_pc;
    ...
    for (int i = 0; i < nb_clusters; i++)
    {
        ...
        {
            static_pc.push_back(pcl::PointXYZL(c_x+var_nor(), c_y+var_nor(), c_z+var_nor()));
    ...
    {
        tmp_pc.push_back(pcl::PointXYZL(i,i,i));
    }
    ...
    NDTMatcherD2D<pcl::PointXYZL,pcl::PointXYZL> matcher(use_irregular_grid,!use_singleres,resolutions);
    ...
    }
    {
        ...
        SpatialIndex<pcl::PointXYZL>* index = NULL;
        if (use_lazzy)
        {
            index = new LazyGrid<pcl::PointXYZL>(current_resolution);
        }
        else
        {
            index = new CellVector<pcl::PointXYZL>();
        }

        NDTMap<pcl::PointXYZL> ndt(index);
        if (!use_lazzy)
        ...
        NDTMap<pcl::PointXYZL> mov(index);
        if (!use_lazzy)
        ...
        if (use_featurecorr)
        {
            NDTMatcherFeatureD2D<pcl::PointXYZL,pcl::PointXYZL> matcher_feat(corresp);
