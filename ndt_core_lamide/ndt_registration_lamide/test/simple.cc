...
main (int argc, char** argv)
{
    ...
    pcl::PointCloud<pcl::PointXYZL> cloud, cloud_offset, cloud_trans;
    char fname[50];
    FILE *fout;
    ...
    if(argc == 9)
    {
        ...
        //ORU_TODO fix these to load pcd files
	if (pcl::io::loadPCDFile<pcl::PointXYZL> (argv[7], cloud) == -1) //* load the file
	{
	    std::cerr<<"Couldn't read file\n";
	    return (-1);
	}
	if (pcl::io::loadPCDFile<pcl::PointXYZL> (argv[8], cloud_offset) == -1) //* load the file
	{
	    std::cerr<<"Couldn't read file\n";
	    return (-1);
	}
	...
	//lslgeneric::NDTMatcherD2D_2D<pcl::PointXYZL,pcl::PointXYZL> matcherD2D(false, false, resolutions);
	...
	pcl::PointCloud<pcl::PointXYZRGBL> cloud_comb;
	pcl::PointXYZRGBL red(255,0,0,0);
	...
	pcl::PointXYZRGBL green(0,200,0,0);
	...
	pcl::PointXYZRGBL blue(10,20,200,0);
