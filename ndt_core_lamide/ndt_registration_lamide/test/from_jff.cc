...
main (int argc, char** argv)
{
    ...
    perception_oru::NDTMap<pcl::PointXYZLI> ndmap(new perception_oru::LazyGrid<pcl::PointXYZLI>(0.4)), local_map(new perception_oru::LazyGrid<pcl::PointXYZLI>(0.4));
    ndmap.loadFromJFF(argv[7]);
    local_map.loadFromJFF(argv[8]);

    //lslgeneric::NDTMatcherD2D_2D<pcl::PointXYZLI,pcl::PointXYZLI> matcherD2D(false, false, resolutions);
    perception_oru::NDTMatcherD2D<pcl::PointXYZLI,pcl::PointXYZLI> matcherD2D(false, false, resolutions);
