...
/*
static int ctr = 0;
bool matchICP(pcl::PointCloud<pcl::PointXYZL> &fixed,  pcl::PointCloud<pcl::PointXYZL> &moving,
    ...
    pcl::PointCloud<pcl::PointXYZL>::ConstPtr f (new pcl::PointCloud<pcl::PointXYZL>(fixed) );
    pcl::PointCloud<pcl::PointXYZL>::ConstPtr m (new pcl::PointCloud<pcl::PointXYZL>(moving) );

    pcl::VoxelGrid<pcl::PointXYZL> gr1,gr2;
...
//    cout<<"epsilon : "<<icp.getTransformationEpsilon() << endl;
    pcl::PointCloud<pcl::PointXYZL> Final;
    icp.align(Final);
...
main (int argc, char** argv)
{
    ...
    pcl::PointCloud<pcl::PointXYZL> cloud, cloud_offset, cloud_OFF;
    ...
    lslgeneric::NDTMatcherP2D<pcl::PointXYZL,pcl::PointXYZL> matcherP2D(resolutions);
    ...
    if(argc == 3)
    {
        ...
        cloud_offset = lslgeneric::readVRML<pcl::PointXYZL>(argv[2]);
        /*
        ...
        /*
        	...
        	FILE *fout = fopen(fname,"w");
        	fprintf(fout,"#VRML V2.0 utf8\n");
        	lslgeneric::writeToVRML<pcl::PointXYZL>(fout,cloud,Eigen::Vector3d(0,1,0));
        	lslgeneric::writeToVRML<pcl::PointXYZL>(fout,cloud_offset,Eigen::Vector3d(1,0,0));
        	fclose(fout);
        */
        lslgeneric::NDTMatcherD2D<pcl::PointXYZL,pcl::PointXYZL> matcherD2D(false, false, resolutions);
        bool ret = matcherD2D.match2D(cloud,cloud_offset,Tout);

        snprintf(fname,49,"c_offset.wrl");
        fout = fopen(fname,"w");
        fprintf(fout,"#VRML V2.0 utf8\n");
        lslgeneric::writeToVRML<pcl::PointXYZL>(fout,cloud,Eigen::Vector3d(1,0,0));
        lslgeneric::writeToVRML<pcl::PointXYZL>(fout,cloud_offset,Eigen::Vector3d(1,1,1));

        lslgeneric::transformPointCloudInPlace<pcl::PointXYZL>(Tout,cloud_offset);
        std::cout<<Tout.matrix()<<std::endl;
        lslgeneric::writeToVRML<pcl::PointXYZL>(fout,cloud_offset,Eigen::Vector3d(0,1,0));
        fclose(fout);
        ...
        sensor_origin<<0,0,0;
        lslgeneric::NDTMap<pcl::PointXYZL> sourceNDT(new lslgeneric::LazyGrid<pcl::PointXYZL>(0.5));
        //sourceNDT.loadPointCloud(cloud);
        sourceNDT.addPointCloud(sensor_origin,cloud);
        sourceNDT.computeNDTCells();
        Tin.setIdentity();

        lslgeneric::NDTMap<pcl::PointXYZL> targetNDT(new lslgeneric::LazyGrid<pcl::PointXYZL>(0.5));
...
#if 0
std::vector<lslgeneric::NDTCell<pcl::PointXYZL>*> source = sourceNDT.pseudoTransformNDT(Tin);
int N_X=40;
int N_Y=40;
...
#pragma omp parallel
{
    lslgeneric::NDTMatcherD2D<pcl::PointXYZL,pcl::PointXYZL> matcherD2DLocal(false, false, resolutions);
    #pragma omp for
...
//		    double xoffset = -0.5, yoffset=0.3, yawoffset=-0.2;
            ...
            pcl::PointCloud<pcl::PointXYZL> cloudL = lslgeneric::transformPointCloud<pcl::PointXYZL>(Tloc,cloud_offset);
            lslgeneric::NDTMap<pcl::PointXYZL> targetNDT(new lslgeneric::LazyGrid<pcl::PointXYZL>(0.5));
            ...
            /*
            		    lslgeneric::transformPointCloudInPlace<pcl::PointXYZL>(Tout,cloudL);
            		    snprintf(fname,49,"c_offset_l.wrl");
            		    fout = fopen(fname,"w");
            		    fprintf(fout,"#VRML V2.0 utf8\n");
            		    lslgeneric::writeToVRML<pcl::PointXYZL>(fout,cloud,Eigen::Vector3d(0,1,0));
            		    lslgeneric::writeToVRML<pcl::PointXYZL>(fout,cloudL,Eigen::Vector3d(1,0,0));
            		    fclose(fout);
            ...
            */
            ...
            std::vector<lslgeneric::NDTCell<pcl::PointXYZL>*> source = sourceNDT.pseudoTransformNDT(Tlocal);
...
//	std::cout<< " H2 = ["<<H2<<"];\n";
return 0;
/*	pcl::PointCloud<pcl::PointXYZL> cloud1, cloud_offset1;
