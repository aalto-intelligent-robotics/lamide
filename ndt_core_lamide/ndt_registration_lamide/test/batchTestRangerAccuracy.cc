...
pcl::PointCloud<pcl::PointXYZL> generateNegatives(pcl::PointCloud<pcl::PointXYZL> &cloud)
{

    pcl::PointCloud<pcl::PointXYZL> negCloud;
    double min_offset = 0.1;
    double max_offset = 2;
    pcl::PointXYZL origin, neg;
...
int main(int argc, char **argv)
{
    ...
    pcl::PointCloud<pcl::PointXYZL> cloudLaser, negLaser;
    pcl::PointCloud<pcl::PointXYZL> cloudKinect, negKinect;
    pcl::PointCloud<pcl::PointXYZL> cloudSR, negSR;
    pcl::PointCloud<pcl::PointXYZL> cloudFotonic, negFotonic;
    pcl::PointCloud<pcl::PointXYZL> gt, negGt;
    ...
    for(int i=0; i<N_CLOUDS; i++)
    {
        ...
        if(doMatch)
        {
            ...
            if(!doGroundTruth)
            {
                ...
                {
                    pcl::PointXYZL pt = cloudLaser.points[q];
            ...
            {
                pcl::PointXYZL pt = cloudKinect.points[q];
                double angle_x = atan2(pt.x,pt.y);
                double angle_y = atan2(pt.x,pt.z);
                double angle_z = atan2(pt.y,pt.z);
                double dist = sqrt(pt.x*pt.x + pt.y*pt.y + pt.z*pt.z);
            ...
            {
                pcl::PointXYZL pt = cloudSR.points[q];
                double angle_x = atan2(pt.x,pt.y);
                double angle_y = atan2(pt.x,pt.z);
                double angle_z = atan2(pt.y,pt.z);
                double dist = sqrt(pt.x*pt.x + pt.y*pt.y + pt.z*pt.z);
            ...
            {
                pcl::PointXYZL pt = cloudFotonic.points[q];
                double angle_x = atan2(pt.x,pt.y);
                double angle_y = atan2(pt.x,pt.z);
                double angle_z = atan2(pt.y,pt.z);
                //cout<<angle_x<<" "<<angle_y<<" "<<angle_z<<endl;
            ...
            if(doGroundTruth)
            {
                pcl::PointCloud<pcl::PointXYZL> gt2;
                for(int q=0; q<gt.points.size(); q++)
                {
                    pcl::PointXYZL pt = gt.points[q];
                    double angle_x = atan2(pt.x,pt.y);
                    double angle_y = atan2(pt.x,pt.z);
                    double angle_z = atan2(pt.y,pt.z);
                    //cout<<angle_x<<" "<<angle_y<<" "<<angle_z<<endl;
