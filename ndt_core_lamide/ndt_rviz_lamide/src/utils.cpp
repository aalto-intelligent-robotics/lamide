#include <ndt_rviz_lamide/utils.h>
...
{
    // To simply get the yaw from the euler angles is super sensitive to numerical
    // errors which will cause roll and pitch to have angles very close to PI...
...
Eigen::Transform<double, 3, Eigen::Affine, Eigen::ColMajor> ICPwithCorrMatch(
    perception_oru::NDTMap& targetNDT,
...
void printTransf2d(const Eigen::Transform<double, 3, Eigen::Affine, Eigen::ColMajor>& T)
{
    std::cout << "[" << T.translation()[0] << "," << T.translation()[1]
              ...
              << ndt_rviz::getRobustYawFromAffine3d(T) << ")" << std::endl;
}
...
void setNDTCellToMap(NDTMap* map, NDTCell* cell)
{
    ...
    if (lz != NULL)
    {
        ...
        if (c == NULL)
        {
            // Needs some means to create it... do this by adding a point the returned
            // cell will be an nice an allocated one.
            c = lz->addPoint(pt_mean);
        }
        //    c->setParameters(0.1, 8*M_PI/18, 1000); //Increase how small the
        //    covariance could be.
...
bool discardCell(NDTMap& map, const pcl::PointXYZL& pt)
{
    NDTCell* cell;
...
// int discardDistantCells(NDTMap &map, const pcl::PointCloud<pcl::PointXYZL>
// &pts, double max_dist) {
    ...
    const Eigen::Transform<double, 3, Eigen::Affine, Eigen::ColMajor>& T)
{
    ...
    Eigen::Affine3d T2 = Eigen::Translation<double, 3>(T.translation()[0], T.translation()[1], 0.) *
