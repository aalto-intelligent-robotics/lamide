...
/** constructor, calls parent OctTree constructor
  */
template <typename PointT>
AdaptiveOctTree<PointT>::AdaptiveOctTree(pcl::PointXYZL center, double xsize, double ysize,
...
std::vector<OctTree<PointT>*> AdaptiveOctTree<PointT>::splitTree(OctTree<PointT> *octLeaf)
{
    ...
    if(octLeaf->isLeaf())
    {
        ...
        pcl::PointXYZL myCenter = octLeaf->myCell_->getCenter();

        //branch leaf
        for(unsigned int it=0; it<8; it++)
        {

            pcl::PointXYZL newCenter;
