...
CellVector::CellVector():mp(new pcl::PointCloud<pcl::PointXYZL>())
{
...
CellVector::CellVector(NDTCell* cellPrototype):mp(new pcl::PointCloud<pcl::PointXYZL>())
{
...
NDTCell* CellVector::getCellForPoint(const pcl::PointXYZL &point)
{
    NDTCell* ret = NULL;
    if (treeUpdated)
    {
...
NDTCell* CellVector::addPoint(const pcl::PointXYZL &point)
{
    return NULL;
    // Do nothing...
}

void CellVector::addCellPoints(pcl::PointCloud<pcl::PointXYZL> pc, const std::vector<size_t> &indices)
{
...
void CellVector::getNeighbors(const pcl::PointXYZL &point, const double &radius, std::vector<NDTCell*> &cells)
{
...
void CellVector::initKDTree()
{
    ...
    pcl::PointXYZL curr;
    Eigen::Vector3d m;
    pcl::PointCloud<pcl::PointXYZL> mc;
...
NDTCell* CellVector::getClosestNDTCell(const pcl::PointXYZL &point)
{
    return getCellForPoint(point);
}

std::vector<NDTCell*> CellVector::getClosestNDTCells(const pcl::PointXYZL &point, double &radius)
{
