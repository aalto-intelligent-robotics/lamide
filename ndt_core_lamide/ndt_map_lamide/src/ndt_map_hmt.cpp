    ...
    if (getline(&line, &len, meta_f) > 0)
    {
        ...
        if (strncmp(tk, "VERSION", 7) == 0)
        {
            ...
            if (strncmp(tk, "2.0", 3) == 0)
            {
                ...
                {
                    std::cerr
                        << "cannot load map, different grid size used... reverting to empty map\n";
                    return false;
                }
            }
            // add cases ...
 ...
 */
void NDTMapHMT::loadPointCloud(const pcl::PointCloud<pcl::PointXYZL>& pc, double range_limit)
{

    // std::cout<<"LOAD pc\n";
    typename pcl::PointCloud<pcl::PointXYZL>::const_iterator it = pc.points.begin();
    NDTCell* ptCell;
                              ...
                              double occupancy_limit)
{
    ...
    while (it != pc.points.end())
    {
        ...
        endpoint << it->x, it->y, it->z;
        if (!lz->traceLineWithEndpoint(origin, endpoint, maxz, cells, pf))
        {
            it++;
            continue;
        }
        ...
        if ((diff2).norm() > eps)
        {
            ...
            if (!grid_[i + 1][j + 1]->traceLineWithEndpoint(pf, endpoint, maxz, cells, pf))
            {
            ...
            // ORU_TODO: we might need to make one more raytrace here
        }

        //		for(unsigned int i=0; i<cells.size(); i++)
        for (auto ptCell : cells)
        {
            //		    ptCell = cells[i];
            if (ptCell != NULL)
            {
                ...
                if (ptCell->hasGaussian_)
                {
                    ...
                    pend << it->x, it->y, it->z;
                    out << pout.x, pout.y, pout.z;
                    double lik = ptCell->computeMaximumLikelihoodAlongLine(po, *it, pout);
                    ...
                    double thr =
                        exp(-0.5 * (l2target * l2target) /
                            (snoise *
                             snoise)); // This is the probability of max lik point being endpoint
                    lik *= (1.0 - thr);
                    if (lik < 0.3)
                        continue;
                                        ...
                                        double sensor_noise)
{
    ...
    // "<<sensor_noise<<std::endl; fprintf(stderr,"Mean nasty point cloud hmt\n");
    if (isFirstLoad_)
    {
    ...
    for (unsigned int it = 0; it < ndts.size(); it++)
    {
        ...
        cells.clear();
        if (!lz->traceLineWithEndpoint(origin, m, maxz, cells, pf))
        {
            it++;
            continue;
        }
        ...
        if ((diff2).norm() > eps)
        {
            ...
            //	    std::vector< NDTCell*> cells2;
            if (!grid_[i + 1][j + 1]->traceLineWithEndpoint(pf, m, maxz, cells, pf))
            {
        ...
        if (updatePositive)
        {
            ...
            addDistributionToCell(ucov, m, numpoints /*, r, g,b*/, maxnumpoints,
                                  occupancy_limit); ///< ORU_FIXME:: local implementation can be faster?
        }
    }
 ...
 */
void NDTMapHMT::addDistributionToCell(const Eigen::Matrix3d& ucov,
                                      const Eigen::Vector3d& umean,
                                      unsigned int numpointsindistribution,
                                      /*float r, float g,float b,*/ unsigned int maxnumpoints,
                                      float max_occupancy)
{
    pcl::PointXYZL pt;
    pt.x = umean[0];
    ...
    if (grid_[i + 1][j + 1]->isInside(pt))
    {
        ...
        if (ptCell != NULL)
        {
            ptCell->updateSampleVariance(ucov, umean, numpointsindistribution, true, 0.6, max_occupancy,
                                         maxnumpoints);
            // ptCell->setRGB(r,g,b);
        }
    }
 ...
 */
void NDTMapHMT::computeNDTCells(int cellupdatemode,
                                unsigned int maxnumpoints,
                                float occupancy_limit,
                                ...
                                double sensor_noise)
{
    ...
    while (it != update_set.end())
    {
        ...
        if (cell != NULL)
        {
            cell->computeGaussian(cellupdatemode, maxnumpoints, occupancy_limit, 0.6, origin,
                                  sensor_noise);
...
bool NDTMapHMT::getCellAtPoint(const pcl::PointXYZL& refPoint, NDTCell*& cell)
{
...
double NDTMapHMT::getLikelihoodForPoint(pcl::PointXYZL pt)
{
...
std::vector<NDTCell*> NDTMapHMT::getInitializedCellsForPoint(const pcl::PointXYZL pt) const
{
...
std::vector<NDTCell*> NDTMapHMT::getCellsForPoint(const pcl::PointXYZL pt,
                                                  int n_neigh,
...
bool NDTMapHMT::getCellForPoint(const pcl::PointXYZL& pt,
                                NDTCell*& out_cell,
                                bool checkForGaussian) const
{
    out_cell = NULL;
...
std::vector<NDTCell*> NDTMapHMT::pseudoTransformNDT(
